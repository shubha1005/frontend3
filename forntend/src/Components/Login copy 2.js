...
const SignInUpForm = () => {
  ...
  const [isAdmin, setIsAdmin] = useState(false); // Track if admin login is selected
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result.user.email === "hackonouts@gmail.com") {
...
.overlay {
	background: #3deb03;
	background: -webkit-linear-gradient(to right, #9DC08B, #9DC08B);
	background: linear-gradient(to right, #9DC08B, #9DC08B);
        ...
        <div className={`form-container sign-up-container ${isSignUp ? "active" : ""}`}>
          <form onSubmit={handleSignUp}>
            ...
            <div className="social-container">
              <p className="social" onClick={handleGoogleSignIn}>
      ...
      <div className={`form-container sign-in-container ${!isSignUp ? "active" : ""}`}>
        <form onSubmit={handleSignIn}>
          ...
          <input 
            ...
            required 
          />
          <button type="submit">{isAdmin ? "Login as Admin" : "Sign In"}</button>
        </form>
      </div>

      {/* Overlay for UI toggle */}
